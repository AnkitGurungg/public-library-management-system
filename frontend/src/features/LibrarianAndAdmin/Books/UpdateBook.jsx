...
const UpdateBook = ({ id }) => {
  ...
  const { data: book, error, isLoading } = useFetchBookById(id, open);
          ...
          {book ? (
            ...
            error && (
              <p className="flex items-center justify-center h-40 text-red-500">
                Error loading shelf: {error?.message || "Something went wrong"}
              </p>
            )
          )}
        </ScrollArea>
