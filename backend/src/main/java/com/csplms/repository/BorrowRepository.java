...
public interface BorrowRepository extends JpaRepository<Borrow, Integer> {
    ...
    @Query(value = "SELECT br.* FROM borrow_records br JOIN (SELECT book_id, COUNT(*) AS borrow_count, MAX(borrow_date) AS latest_borrow_date FROM borrow_records GROUP BY book_id) AS book_stats ON br.book_id = book_stats.book_id GROUP BY br.book_id ORDER BY book_stats.borrow_count DESC, book_stats.latest_borrow_date DESC, br.book_id desc;", nativeQuery = true)
    List<Borrow> topBorrowedBooks();
